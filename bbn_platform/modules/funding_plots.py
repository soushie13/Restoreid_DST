...
def transition_curve_plot(system, phase1, phase2, current_t, scenario_label):
    ...
    ax.axhline(risk_p1, color=MUTED, linestyle=":", linewidth=1.0)
    ax.axhline(risk_p2, color=MUTED, linestyle=":", linewidth=1.0)
    ax.text(14.7, risk_p1, f"Pure {phase1}", fontsize=7.5, color=MUTED,
            ha="right", va="bottom")
    ax.text(14.7, risk_p2, f"Pure {phase2}", fontsize=7.5, color=MUTED,
            ha="right", va="top")
    ...
    ax.set_title(f"{meta['label']} — {scenario_label}",
                 fontsize=10.5, color=TEAL, pad=10)
...
def multi_system_transition_plot(phase1, phase2, current_t):
    ...
    ax.axvline(current_t, color=MUTED, linestyle="--", linewidth=1.0, alpha=0.6)
