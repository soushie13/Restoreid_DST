...
def disease_risk_plot(model_data: dict) -> str:
    ...
    # Baseline
    ax.axhline(baseline, color=MUTED, linestyle="--", linewidth=1.0, zorder=2)
...
def regulation_plot(model_data: dict) -> str:
    ...
    ax.set_title("Ecological regulation recovery", fontsize=10, color=TEAL, pad=8)
    _apply_style(ax, col)
    fig.tight_layout()
    return _fig_to_html(fig)
...
def stability_trajectory_plot(model_data: dict) -> str:
    ...
    ax.set_title("Time to stabilisation by scenario", fontsize=10, color=TEAL, pad=8)
